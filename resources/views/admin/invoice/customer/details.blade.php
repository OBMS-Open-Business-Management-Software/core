...
@section('content')
    <div class="container-fluid my-4">
        <div class="row">
            <div class="col-md-12">
                <a href="{{ route('admin.invoices.customers') }}" class="btn btn-outline-primary mb-4"><i class="bi bi-arrow-left-circle"></i> {{ __('Back to list') }}</a>
                <a href="{{ route('admin.customers.profile', $invoice->user_id) }}" class="btn btn-primary mb-4" target="_blank"><i class="bi bi-person"></i> {{ __('Customer') }}</a>
                @if (! empty($invoice->original) && $invoice->status == 'refund')
                    <a href="{{ route('admin.invoices.customers.details', $invoice->original->id) }}" class="btn btn-primary mb-4"><i class="bi bi-file-earmark-text"></i> {{ __('Linked Invoice (Original)') }}</a>
                @endif
                @if (! empty($invoice->refunded) && $invoice->status == 'refunded')
                    <a href="{{ route('admin.invoices.customers.details', $invoice->refunded->id) }}" class="btn btn-primary mb-4"><i class="bi bi-file-earmark-text"></i> {{ __('Linked Invoice (Refund)') }}</a>
                @endif
                @if ($invoice->status == 'template')
                    ...
                    @if ($invoice->positionLinks->isNotEmpty())
                        <a href="{{ route('admin.invoices.customers.publish', $invoice->id) }}" class="btn btn-primary mb-4 mr-1 float-right"><i class="bi bi-check-circle"></i> {{ __('Publish') }}</a>
                    @endif
                @else
                    @if ($invoice->status == 'paid')
                        <a href="{{ route('admin.invoices.customers.unpaid', $invoice->id) }}" class="btn btn-warning mb-4 mr-1 float-right"><i class="bi bi-x-circle"></i> {{ __('Unpaid') }}</a>
                    @endif
                    @if ($invoice->status == 'unpaid')
                        <a href="{{ route('admin.invoices.customers.paid', $invoice->id) }}" class="btn btn-success mb-4 mr-1 float-right"><i class="bi bi-check-circle"></i> {{ __('Paid') }}</a>
                        <a href="{{ route('admin.invoices.customers.revoke', $invoice->id) }}" class="btn btn-warning mb-4 mr-1 float-right"><i class="bi bi-dash-circle"></i> {{ __('Revoke') }}</a>
                    @endif
                    @if ($invoice->status == 'paid')
                        <a href="{{ route('admin.invoices.customers.refund', $invoice->id) }}" class="btn btn-warning mb-4 mr-1 float-right"><i class="bi bi-dash-circle"></i> {{ __('Refund') }}</a>
                    @endif
                    <a href="{{ route('admin.invoices.customers.download', $invoice->id) }}" class="btn btn-primary mb-4 mr-1 float-right" download><i class="bi bi-download"></i> {{ __('Download') }}</a>
                    <a href="{{ route('admin.invoices.customers.resend', $invoice->id) }}" class="btn btn-primary mb-4 mr-1 float-right"><i class="bi bi-envelope"></i> {{ __('Resend') }}</a>
                @endif
            </div>
        </div>
        <div class="row">
                ...
                <div class="card">
                    ...
                    <div class="card-body">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-9 col-form-label">
                                {!! $invoice->user->locked ? '<span class="badge badge-warning badge-pill">' . __('Locked') . '</span>' : '<span class="badge badge-success badge-pill">' . __('Unlocked') . '</span>' !!}
                            </div>
                        </div>
                                    ...
                                    @foreach ($invoice->positionLinks as $link)
                                        <tr>
                                            ...
                                            @if ($invoice->status == 'template')
                                                ...
                                                <td style="width: 1%">
                                                    <a href="{{ route('admin.invoices.customers.positions.delete', ['id' => $invoice->id, 'position_id' => $link->id]) }}" class="btn btn-danger btn-sm"><i class="bi bi-trash"></i></a>
                                                </td>
                                            @endif
                                        </tr>
    ...
    <script type="text/javascript">
        $(window).on('load', function () {
            ...
            $('#history').DataTable({
                processing: true,
                serverSide: true,
                ajax: '/admin/invoices/customers/{{ $invoice->id }}/history',
                columns: [
                    { data: 'id', sWidth: '1%' },
                    { data: 'date' },
                    { data: 'name' },
