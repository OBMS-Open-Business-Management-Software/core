...
@section('content')
    <div class="container-fluid my-4">
        <div class="row">
            <div class="col-md-12">
                <a class="btn btn-primary float-right mb-4" data-toggle="modal" data-target="#add"><i class="bi bi-plus-circle"></i> {{ __('interface.discounts.create') }}</a>
            </div>
        </div>
        <div class="row">
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">
                        <i class="bi bi-list"></i> {{ __('interface.discounts.list') }}
                    </div>
                    <div class="card-body">
                        <table id="discounts" class="table mt-4 w-100">
                            <thead>
                            <tr>
        ...
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header bg-primary text-white">
                    <h5 class="modal-title" id="addLabel"><i class="bi bi-plus-circle"></i> {{ __('interface.discounts.create') }}</h5>
                        ...
                        <div class="form-group row">
                            ...
                            <div class="col-md-8">
                                <select id="type" type="text" class="form-control @error('type') is-invalid @enderror" name="type">
                                    <option value="fixed"{{ old('type') == 'fixed' ? ' selected' : '' }}>{{ __('interface.units.fixed') }}</option>
                                    <option value="percentage"{{ old('type') == 'percentage' ? ' selected' : '' }}>{{ __('interface.units.percentage') }}</option>
                                </select>
        ...
        $(window).on('load', function () {
            ...
            $('#discounts').DataTable({
                ...
                fnDrawCallback: function () {
                    $('.type_dynamic').each(function () {
                        $(this).on('change', function () {
                            if ($(this).val() == 'fixed') {
                                $('#typeSuffixDynamic' + $(this).attr('data-id')).html('€');
                            } else if ($(this).val() == 'percentage') {
                                $('#typeSuffixDynamic' + $(this).attr('data-id')).html('%');
                            }
                        });
