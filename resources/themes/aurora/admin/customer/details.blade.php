...
@section('content')
    <div class="container-fluid my-4">
        <div class="row">
            <div class="col-md-12">
                <a href="{{ route('admin.customers') }}" class="btn btn-outline-primary mb-4"><i class="bi bi-arrow-left-circle"></i> {{ __('interface.actions.back_to_list') }}</a>
                ...
                @if ($user->locked)
                    <a href="{{ route('admin.customers.lock', $user->id) }}" class="btn btn-success float-right ml-1"><i class="bi bi-unlock"></i> {{ __('interface.actions.unlock') }}</a>
                @else
                    <a href="{{ route('admin.customers.lock', $user->id) }}" class="btn btn-warning float-right ml-1"><i class="bi bi-lock"></i> {{ __('interface.actions.lock') }}</a>
                @endif
                <a class="btn btn-warning float-right ml-1" data-toggle="modal" data-target="#password"><i class="bi bi-key-fill"></i> {{ __('interface.actions.change_password') }}</a>
            </div>
        </div>
        <div class="row">
            <div class="col-md-12">
                <div class="row">
                    ...
                    <div class="col-md-8">
                        <div class="card">
                            <div class="card-header">
                                <i class="bi bi-list"></i> {{ __('interface.misc.accepted_pages') }}
                            </div>
                            <div class="card-body">
                                <table class="table">
                                    <thead>
                                        <tr>
                                            <td>{{ __('interface.data.page') }}</td>
                                            <td>{{ __('interface.data.signature') }}</td>
                                            <td>{{ __('interface.data.ip_address') }}</td>
                                            <td width="50%">{{ __('interface.data.user_agent') }}</td>
                                    ...
                                    <tbody>
                                        @foreach ($user->acceptance as $acceptance)
                                            <tr>
                                                ...
                                                <td>
                                                    {{ ! empty($acceptance->signed_at) ? $acceptance->signed_at->format('d.m.Y, H:i') : __('interface.misc.never') }}<br>
                                                    {!! $acceptance->signed ? '<span class="badge badge-success">' . __('interface.status.valid') . '</span>' : '<span class="badge badge-warning">' . __('interface.status.invalid') . '</span>' !!}<br>
                                                    <span class="small">{{ $acceptance->signature }}</span>
                                                </td>
                                                <td>{{ $acceptance->ip }}</td>
                                                <td>{{ $acceptance->user_agent }}</td>
                                                <td width="1%"><a href="{{ route('cms.page.' . $acceptance->pageVersion->page->id . '.version', $acceptance->pageVersion->id) }}" class="btn btn-primary btn-sm" target="_blank"><i class="bi bi-eye"></i></a></td>
                                            </tr>
                                        @endforeach
                                    </tbody>
                                </table>
                ...
                @if($user->two_factor_confirmed)
                    <div class="card mt-4">
                        ...
                        <div class="card-body">
                            <form action="{{ route('admin.customers.profile.2fa.disable', $user->id) }}" method="post">
                                @csrf
                                @method('delete')
                ...
                <div class="card mt-4">
                    ...
                    <div class="card-body">
                        ...
                        @else
                            ...
                            @endif
                            <label class="font-weight-bold mb-0">{{ __('interface.data.reverse_charge') }}:</label> {!! $user->reverseCharge ? '<span class="badge badge-success">' . __('interface.misc.applicable') . '</span>' : '<span class="badge badge-warning">' . __('interface.misc.not_applicable') . '</span>' !!}<br>
                ...
                <div class="row mt-4">
                    ...
                    <div class="col-md-6">
                        <div class="card">
                            <div class="card-header">
                                <i class="bi bi-file-earmark-text-fill"></i> {{ __('interface.misc.contracts') }}
                            </div>
                            <div class="card-body">
                                <div class="row">
                                    <div class="col-md-12">
                                        <a class="btn btn-primary float-right" data-toggle="modal" data-target="#addContract"><i class="bi bi-plus-circle"></i> {{ __('interface.contracts.create') }}</a>
                                    </div>
                                </div>
                                <div class="row mt-4">
                                    <div class="col-md-12">
    ...
    <div class="modal fade" id="addContract" tabindex="-1" aria-labelledby="addContractLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-header bg-primary text-white">
                    <h5 class="modal-title" id="addContractLabel"><i class="bi bi-plus-circle"></i> {{ __('interface.contracts.create') }}</h5>
                    <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                        <span aria-hidden="true">&times;</span>
                    </button>
                </div>
